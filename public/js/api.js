const API = (() => {
  ...
  return {
    ...
    buyCar: (carId) => token
      ? req('/garage/buy-car', { method: 'POST', body: JSON.stringify({ carId }) })
      : Promise.reject(new Error('Sign in to buy cars')),

    claimMission: (missionId) => token
      ? req('/missions/claim', { method: 'POST', body: JSON.stringify({ missionId }) })
      : Promise.reject(new Error('Sign in to claim rewards')),
