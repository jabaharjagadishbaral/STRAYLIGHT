const AUTH_UI = (() => {
  ...
  async function enterGarage() {
    ...
    const uidEl = document.getElementById('driver-uid');
    if (user) {
      ...
      if (uidEl) {
        uidEl.textContent = 'UID ' + String(user.id).slice(0, 8).toUpperCase();
        uidEl.title = 'Full account UID: ' + user.id;
      ...
      if (uidEl) uidEl.classList.add('hidden');
    }
